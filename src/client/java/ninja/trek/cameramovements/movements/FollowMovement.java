...
public class FollowMovement extends AbstractMovementSettings implements ICameraMovement {
    ...
    public MovementState calculateState(MinecraftClient client, Camera camera, float deltaSeconds) {
        ...
        if (resetting) {
            ...
            Vec3d desiredXZ = computeFollowXZ(stickPos, cur, xzThreshold);
            double desiredY = computeFollowY(stickPos, cur.y, followHeight, yThreshold, client.player.isOnGround());

            Vec3d desiredRaw = new Vec3d(desiredXZ.x, desiredY, desiredXZ.z);
    ...
    private Vec3d computeFollowXZ(Vec3d playerPos, Vec3d cameraPos, double threshold) {
        Vec3d delta = new Vec3d(playerPos.x - cameraPos.x, 0.0, playerPos.z - cameraPos.z);
        double dist = delta.length();
        if (dist <= threshold || dist <= 1e-9) {
            return new Vec3d(cameraPos.x, 0.0, cameraPos.z);
        }

        Vec3d dir = delta.multiply(1.0 / dist);
        Vec3d desiredCam = new Vec3d(playerPos.x, 0.0, playerPos.z).subtract(dir.multiply(threshold));
        return new Vec3d(desiredCam.x, 0.0, desiredCam.z);
    }

    private double computeFollowY(Vec3d playerPos, double currentY, double height, double threshold, boolean onGround) {
        double targetY = playerPos.y + height;
        if (onGround) {
            return targetY;
        }
