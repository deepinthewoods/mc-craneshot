...
- Minecraft: 26.2 (official Mojang mappings resolved by Loom)
- Fabric API docs: https://maven.fabricmc.net/docs/fabric-api-0.156.0+26.2/
- Fabric Loader docs: https://maven.fabricmc.net/docs/fabric-loader-0.19.3/

Before Writing or Modifying Mixins
- Verify method signatures: Inspect the exact official-mapped signature (name, params, return) for the current `minecraft_version` in Loom's decompiled sources or the resolved Minecraft jars. Do not guess parameter lists.
...
- Search the linked Fabric API/Loader docs and Loom's official-mapped Minecraft sources first. Cross-check changed API signatures, removed methods, or renamed classes for the current versions in `gradle.properties`.
